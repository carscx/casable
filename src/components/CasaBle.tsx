...
function IconBulb({ on=false }: { on?: boolean }) {
  return (
    <svg className={`icon-bulb ${on ? "on" : ""}`} viewBox="0 0 24 24" width="22" height="22" aria-hidden>
...
function IconFan({ level=0 }: { level?: number }) {
  return (
    <svg className={`icon-fan lvl-${level}`} viewBox="0 0 24 24" width="22" height="22" aria-hidden>
...
function useBle() {
  ...
  function disconnect(){ deviceRef.current?.gatt?.disconnect(); }

  async function send(cmd: string) {
    if (!rxRef.current) return;
    const data = new TextEncoder().encode(cmd);
    // @ts-ignore
    if (rxRef.current.writeValueWithoutResponse) await rxRef.current.writeValueWithoutResponse(data);
...
export default function CasaBle() {
  ...
  async function toggle(zone: "PASILLO"|"COCINA"|"DORMITORIO"|"ASEO"|"LIVING") {
    const mapState = { PASILLO: pasillo, COCINA: cocina, DORMITORIO: dormitorio, ASEO: aseo, LIVING: living } as const;
    const setters  = { PASILLO: setPasillo, COCINA: setCocina, DORMITORIO: setDormitorio, ASEO: setAseo, LIVING: setLiving } as const;
  ...
  async function all(on:boolean){
    setPasillo(on); setCocina(on); setDormitorio(on); setAseo(on); setLiving(on); setVent(on?2:0);
  ...
  return (
    <div className="app">
      ...
      <div className={`plano ${connected ? "" : "disabled"}`}>
        <div className={`zona pasillo ${pasillo?"on":""}`} onClick={()=>connected && toggle("PASILLO")}>
          <IconBulb on={pasillo} /><span>Pasillo</span>
        </div>
        <div className={`zona cocina ${cocina?"on":""}`} onClick={()=>connected && toggle("COCINA")}>
          <IconBulb on={cocina} /><span>Cocina</span>
        </div>

        <div className={`zona dormitorio ${dormitorio?"on":""}`} onClick={()=>connected && toggle("DORMITORIO")}>
          <IconBulb on={dormitorio} /><span>Dormitorio</span>
        </div>
        <div className={`zona bano ${aseo?"on":""}`} onClick={()=>connected && toggle("ASEO")}>
          <IconBulb on={aseo} /><span>Baño</span>
        </div>

        <div className={`zona living ${living?"on":""}`} onClick={()=>connected && toggle("LIVING")}>
          <IconBulb on={living} /><span>Living</span>
        </div>

        <div className={`zona comedor nivel-${vent}`} onClick={()=>connected && cycleVent()}>
          <div className="ventilador" onClick={(e)=>e.stopPropagation()}>
            <div className="etiqueta">
              <IconFan level={vent} /> Ventilador Comedor
            </div>
            <div className="stepper">
              <button className="btn" disabled={!connected || vent<=0} onClick={()=>stepVent(-1)}>–</button>
              <strong>{vent===0?"OFF":`Vel ${vent}`}</strong>
              <button className="btn" disabled={!connected || vent>=3} onClick={()=>stepVent(+1)}>+</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="acciones">
        <button className="btn" disabled={!connected} onClick={()=>all(true)}>Todo ON</button>
        <button className="btn" disabled={!connected} onClick={()=>all(false)}>Todo OFF</button>
      </div>
    </div>
  );
}
